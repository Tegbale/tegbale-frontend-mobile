...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'ViewWard'>;
  route: RouteProp<RootStackParamList, 'ViewWard'>;
};
...
const MOCK_COMMENTS = [
  {
    ...
    time: 'Today 14:20pm',
    text: "Your child did great today!",
    commentCount: 1,
    reply: { author: 'You (Guardian)', time: 'Today 14:35pm', text: "That's my child!!" },
    photo: false,
  },
  {
    id: '2',
...
export default function ViewWardModal({ navigation, route }: Props) {
              ...
              <View key={comment.id} style={styles.commentCard}>
                ...
                {comment.reply && (
                  <View style={styles.replyItem}>
                    <AvatarCircle type="parent" size={28} />
                    <View style={styles.replyContent}>
                      <Text style={styles.replyAuthor}>{comment.reply.author}</Text>
                      <Text style={styles.replyText2}>{comment.reply.text}</Text>
                    </View>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  replyItem: { flexDirection: 'row', gap: 8, marginLeft: 20, marginBottom: 8 },
  replyContent: { flex: 1 },
  replyAuthor: { fontSize: 13, fontWeight: 'bold', color: Colors.darkText },
  replyText2: { fontSize: 13, color: Colors.darkText },

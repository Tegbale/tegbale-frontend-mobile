...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'TeacherLogin'>;
};
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (u.role !== 'TEACHER' && u.role !== 'STAFF') {
        await logout();
        Alert.alert('Wrong Portal', 'This login is for teachers only. Please use the Parent login.');
        return;
      }
      navigation.navigate('TeacherApp');
    } catch (e: any) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.titleRow}>
            <AvatarCircle type="teacher" size={52} />
            <Text style={styles.title}>Teacher Login</Text>
          </View>

          <Text style={styles.hint}>
            Use the credentials sent to your school email when your account was created.
          </Text>
